...
This repo packages [Fedimint Guardian](https://github.com/fedimint/fedimint) for StartOS.
...
The `fedimintd` image is built locally from `Dockerfile`, which extends the upstream `fedimint/fedimintd` image. There is no `dockerTag` in the manifest.

1. Bump the `FROM fedimint/fedimintd:v<version>` line in `Dockerfile` to the new upstream tag.
